...
function addTemplateRoutes(app) {
	...
	app.get('/github-login', showGithubLogin)
...
async function signup(request, reply) {
	reply.view('/signup.ejs', {clientId: configuration.githubId, state: await createSecureToken()})
	return reply
}
...
async function showGithubLogin(request, reply) {
	const auth = await loginWithGithub(request)
	console.log(auth)
