...
function checkUserData(data) {
	...
	if (data.password != data.confirmPassword) {
		throw new ApiError(400, `Mismatched passwords`)
	}
}

function checkUsername(username) {
	if (!username || !username.length) throw new ApiError(400, `Missing username`)
	if (username.length < 6) throw new ApiError(400, `Username should have at least 6 characters`)
	...
	if (username.match(/_/)) throw new ApiError(`Username can only have letters and numbers`)
}
