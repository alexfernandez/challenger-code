...
describe('Challenge integration tests', function() {
	...
	it('should find a challenge', async() => {
		const challenge = await request.get(`${base}/challenge/main/${challengeId}`)
	...
	it('should run a challenge', async() => {
		const run = await request.post(`${base}/challenge/main/${challengeId}/run`, {code}, headers)
		checkRun(run)
		expect(run.success).to.equal(true)
	})
	it('should fail a challenge', async() => {
		const run = await request.post(`${base}/challenge/main/${challengeId}/run`, {code: failure}, headers)
		checkRun(run)
		expect(run.success).to.equal(false)
	})
	it('should reject a challenge', async() => {
		try {
			await request.post(`${base}/challenge/main/${challengeId}/run`, {code: wrong}, headers)
	...
	it('should run challenges in parallel', async() => {
		const promises = []
		for (let i = 0; i < 5; i++) {
			const promise = request.post(`${base}/challenge/main/${challengeId}/run`, {code: parallel}, headers)
