...
const data = {
	...
	verifications: [{
		...
		name: 'Autogenerated',
		input: "\"${array}\", \"${number}\"",
		array: 'const array = []; for (let i = 0; i < 1000; i++) array.push(i);',
		number: 'const number = Math.floor(Math.random() * 1000)',
		output: 0,
	}],
	maxMinutes: 5,
...
describe('Challenge model tests', function() {
	...
	it('should create challenge', async() => {
		...
		expect(last).to.have.property('parameters')
		expect(last.parameters.length).to.equal(2)
		const array = last.parameters[0]
		expect(array.length).to.equal(1000)
		for (let i = 0; i < array.length; i++) {
			expect(array[i]).to.equal(i)
		}
		const number = last.parameters[1]
