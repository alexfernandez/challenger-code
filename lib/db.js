...
async function readLast(collection, data) {
	const reversed = await getCollection(collection).find(data).sort({_id: -1}).limit(1).toArray()
	console.log(reversed)
	return reversed[0]
}

async function stats(collection, data, group, fields) {
...
module.exports = {
	start, stop, getUrl,
	insert, update,
	read, readLast, list, listGrouped,
